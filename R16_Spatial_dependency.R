...
# .. note the specific syntax which makes GDP.sf an {sf} object
#
# Plot the data 
ggplot(GDP.sf) +
...
#
#       | GDP value in region | long (representative location) | lat (represent.) |
# Step 1: store long/lat coordinates of centroids
GDP.sp <- as_Spatial(GDP.sf)
Centroids1 <- GDP.sp$NUTS_ID
...
coords <- coordinates(GDP.data[,c("lon", "lat")]) # sp coordinates
